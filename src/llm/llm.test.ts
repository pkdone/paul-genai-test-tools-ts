...
test(`LLMRouter reduce prompt size 1`, () => {
  ...
  expect(llmRouter.TEST_reducePromptSizeToTokenLimit(prompt, GPT_COMPLETIONS_MODEL_GPT4, tokensUage)).toBe("1234 1234 1234 1");
});
...
test(`LLMRouter reduce prompt size 2`, () => {
  ...
  expect(llmRouter.TEST_reducePromptSizeToTokenLimit(prompt, GPT_COMPLETIONS_MODEL_GPT4, tokensUage).length).toBe(150);
});
...
test(`LLMRouter reduce prompt size 3`, () => {
  ...
  expect(llmRouter.TEST_reducePromptSizeToTokenLimit(prompt, GPT_COMPLETIONS_MODEL_GPT4, tokensUage).length).toBe(26209);
 });
