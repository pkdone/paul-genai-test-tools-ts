...
describe("Response Processing tests", () => {
  ...
  describe("Test parseTokenUsageFromLLMError function", () => {
    ...
    describe("Bedrock error patterns", () => {
      ...
      // New tests with reversed isMaxFirst patterns
      test("should extract tokens from reversed 'too many input tokens' error message", () => {
        const errorMsg = "ValidationException: 400 Bad Request: Too many input tokens. Request input token count: 9279, max input tokens: 8192";
        // Create a custom pattern with isMaxFirst set to false for this test
        const reversedPattern = [
          { pattern: /Request input token count.*?(\d+).*?max input tokens.*?(\d+)/, units: "tokens", isMaxFirst: false }
        ];
        const result = parseTokenUsageFromLLMError("GPT_COMPLETIONS_GPT4", errorMsg, testMetadata, reversedPattern);
      ...
      test("should extract chars from reversed 'malformed input request' error message", () => {
        const errorMsg = "ValidationException: Malformed input request: actual: 52611, expected maxLength: 50000, please reformat your input and try again.";
        // Create a custom pattern with isMaxFirst set to false for this test
        const reversedPattern = [
      ...
      test("should extract tokens from reversed 'maximum context length' error message pattern", () => {
        ...
        const errorMsg = "ValidationException: For prompt with at least 9000 tokens, the model's maximum context length is 8192 tokens. Please reduce the length.";
        // Create a custom pattern with isMaxFirst set to false for this test
        const reversedPattern = [
          { pattern: /prompt with at least (\d+) tokens.*?maximum context length is (\d+) tokens/, units: "tokens", isMaxFirst: false }
        ];
        const result = parseTokenUsageFromLLMError("GPT_COMPLETIONS_GPT4", errorMsg, testMetadata, reversedPattern);
      ...
      test("should extract tokens from 'prompt contains tokens' error message with normal order", () => {
        ...
        const errorMsg = "ValidationException. Model has maximum context length of 131072 tokens, but prompt contains 235396 tokens and 0 draft tokens.";
        // Create a custom pattern with isMaxFirst set to true for this test
        const normalOrderPattern = [
          { pattern: /maximum context length of (\d+) tokens.*?prompt contains (\d+) tokens/, units: "tokens", isMaxFirst: true }
        ];
        const result = parseTokenUsageFromLLMError("GPT_COMPLETIONS_GPT4", errorMsg, testMetadata, normalOrderPattern);
    ...
    describe("OpenAI error patterns", () => {
      ...
      // New tests with reversed isMaxFirst patterns for OpenAI
      test("should extract tokens from reversed 'maximum context length with prompt and completion breakdown' error message", () => {
        const errorMsg = "You requested 10346 tokens for prompt and 5 tokens for completion, however this model's maximum context length is 8191 tokens. Please reduce your prompt or completion length.";
        // Create a custom pattern with isMaxFirst set to false for this test
        const reversedPattern = [
          { pattern: /requested (\d+) tokens.*?maximum context length is (\d+) tokens/, units: "tokens", isMaxFirst: false }
        ];
        const result = parseTokenUsageFromLLMError("GPT_COMPLETIONS_GPT4", errorMsg, testMetadata, reversedPattern);
      ...
      test("should extract tokens from reversed 'maximum context length with messages resulted' error message", () => {
        const errorMsg = "Your messages resulted in 8545 tokens. This model's maximum context length is 8192 tokens. Please reduce the length of the messages.";
        // Create a custom pattern with isMaxFirst set to false for this test
        const reversedPattern = [
          { pattern: /messages resulted in (\d+) tokens.*?maximum context length is (\d+) tokens/, units: "tokens", isMaxFirst: false }
        ];
        const result = parseTokenUsageFromLLMError("GPT_COMPLETIONS_GPT4", errorMsg, testMetadata, reversedPattern);
