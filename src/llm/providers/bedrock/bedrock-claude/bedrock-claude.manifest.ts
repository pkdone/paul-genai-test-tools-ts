...
export const bedrockClaudeProviderManifest: LLMProviderManifest = {
  ...
  models: {
    ...
    primaryCompletion: {
      ...
      urn: (env) => {
        const value = env[BEDROCK_CLAUDE_COMPLETIONS_MODEL_PRIMARY_KEY] as string;
        if (!value) throw new Error(`Required environment variable ${BEDROCK_CLAUDE_COMPLETIONS_MODEL_PRIMARY_KEY} is not set`);
        return value;
      },
    ...
    secondaryCompletion: {
      ...
      urn: (env) => {
        const value = env[BEDROCK_CLAUDE_COMPLETIONS_MODEL_SECONDARY_KEY] as string;
        if (!value) throw new Error(`Required environment variable ${BEDROCK_CLAUDE_COMPLETIONS_MODEL_SECONDARY_KEY} is not set`);
        return value;
      },
      purpose: LLMPurpose.COMPLETIONS,
