...
export const appDescriptionSchema = z.object({
  appDescription: z.string().describe("A detailed description of the application's purpose and implementation (max 25 sentences)."),
});
export type AppDescription = z.infer<typeof appDescriptionSchema>;

// Reusable schema for name/description pairs
const nameDescSchema = z.object({
...
export const boundedContextsSchema = z.object({
  boundedContexts: z.array(nameDescSchema).describe("A list of bounded contexts from a Domain Driven Design perspective."),
});
...
export const businessEntitiesSchema = z.object({
  businessEntities: z.array(nameDescSchema).describe("A list of the application's main business entities."),
});
...
export const businessProcessesSchema = z.object({
  businessProcesses: z.array(nameDescSchema).describe("A list of the application's main business processes."),
});
...
export const technologiesSchema = z.object({
  technologies: z.array(nameDescSchema).describe("A list of key external and host platform technologies depended on by the application."),
});
